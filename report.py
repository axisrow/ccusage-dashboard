...
function chart(d, m) {
  ...
  // грануляции, чтобы дни читались визуально. hours при HOUR — исходный почасовой
  // массив DATA.hours; новый день начинается с часа "00". hi > 0: если данные
  // начинаются ровно в полночь, линия на левой рамке области не нужна.
  if (bucketSize === HOUR) {
    hours.forEach((h, hi) => {
