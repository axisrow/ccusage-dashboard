...
4. cost = 0, если ccusage говорит, что модель за период не стоила ничего:
   ...
   тарифицируется claude-opus-5 — по API-ставкам он дал бы фиктивные сотни
   долларов. (glm-5.2 и deepseek-v4-flash раньше тоже выглядели подпиской, но
   это API-модели, которых ccusage ещё не знал, — теперь у них есть ставки
   из FALLBACK/LiteLLM.)
