...
Ставки берутся из трёх источников, в порядке доверия (всё кэшируется в rates.json):
...
1. LiteLLM (тот же прайсинг, что использует сам ccusage). Учитывается TTL кэша:
   ...
   занижался примерно на 9%.
2. Подгонка из ccusage методом наименьших квадратов — для моделей, которых нет в
...
3. cost = 0, если ccusage говорит, что модель за период не стоила ничего:
...
# --------------------------------------------------------------------------- #
# Источник 2: подгонка из ccusage
# --------------------------------------------------------------------------- #
...
def build(
    ...
    for model, rows in observed.items():
        ...
        # есть, она и используется.
        rates = from_litellm(model, prices)
        if rates is not None:
            table[model] = rates
            continue

        # 2. Цены в LiteLLM нет, и ccusage стабильно (не менее трёх дней) отдаёт
        ...
        # 3. подгонка — только для не-Anthropic моделей
